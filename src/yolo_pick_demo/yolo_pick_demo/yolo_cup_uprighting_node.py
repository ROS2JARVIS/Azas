...
ENABLE_INITIAL_OBSERVE_FAST_PICK = os.getenv(
    "YOLO_ENABLE_INITIAL_OBSERVE_FAST_PICK", "1"
).lower() in {"1", "true", "yes", "on"}
INITIAL_OBSERVE_UP_TOL_RAD = np.radians(
    float(os.getenv("YOLO_INITIAL_OBSERVE_UP_TOL_DEG", "30.0"))
)
INITIAL_OBSERVE_GRASP_LEFT_TOL_RAD = np.radians(
    float(os.getenv("YOLO_INITIAL_OBSERVE_GRASP_LEFT_TOL_DEG", "30.0"))
)
...
class YoloCupUprightingNode(BaseMoveItPickNode):
    ...
    def _initial_observe_alignment_ok(self, mouth, grasp_yaw):
        """초기 observe 화면에서 입구 벡터가 12시 근처인지 판단."""
        if not ENABLE_INITIAL_OBSERVE_FAST_PICK:
        ...
        ok = (
            up_error is not None
            and abs(up_error) <= INITIAL_OBSERVE_UP_TOL_RAD
        )
        return ok, up_error, grasp_left_error

    def _safe_z_alignment_ok(self, grasp_yaw):
        """safe_z 도착 후에도 입구 벡터가 12시 근처인지 확인."""
        ...
        mouth_theta = mouth["marker_theta"] if mouth["found"] else estimate.get("mouth_theta")
        if mouth_theta is None:
            return False, None, None, mouth["found"]
        ...
        return ok, up_error, grasp_left_error, mouth["found"]
    ...
    def detect_and_pick(self, frame: np.ndarray):
        ...
        initial_aligned, initial_up_error, initial_grasp_left_error = (
            self._initial_observe_alignment_ok(mouth, grasp_yaw)
        )
        if initial_aligned:
            log.warn(
                "[VISION] 초기 observe에서 입구 벡터 12시 근처 OK. "
                f"up_err={np.degrees(initial_up_error):+.1f}도, "
                f"grasp_left_err={np.degrees(initial_grasp_left_error):+.1f}도. "
                "safe_z 이후 yaw 회전 없이 중앙점 파지를 시도합니다."
            )
        elif initial_up_error is not None and initial_grasp_left_error is not None:
            log.info(
                "[VISION] 초기 observe 입구 벡터 12시 조건 미충족. "
                f"up_err={np.degrees(initial_up_error):+.1f}도, "
                f"grasp_left_err={np.degrees(initial_grasp_left_error):+.1f}도."
            )
        # ==========================================================
        ...
        try:
            self._pick_and_straighten(
                ...
                skip_safe_realign=initial_aligned,
            )
        finally:
    ...
    def _pick_and_straighten(
        self, bx, by, bz, cup_theta, grasp_yaw=None, skip_safe_realign=False
    ):
        log = self.get_logger()
        ...
        if skip_safe_realign:
            safe_ok, safe_up_error, safe_grasp_left_error, marker_found = (
                self._safe_z_alignment_ok(grasp_yaw)
            )
            if safe_ok:
                log.warn(
                    "[1-2] safe_z 도착 후에도 초기 정렬이 유지됩니다. "
                    f"up_err={np.degrees(safe_up_error):+.1f}도, "
                    f"grasp_left_err={np.degrees(safe_grasp_left_error):+.1f}도, "
                    f"marker={'yes' if marker_found else 'no'}. "
                    "yaw 회전 없이 컵 중앙점만 맞춰 파지 준비로 진행합니다."
                )
                grasp_yaw = _normalize_angle(current_yaw)
                close_axis_theta = grasp_yaw + GRIPPER_CLOSE_AXIS_OFFSET_RAD
                target_ori = current_ori

                if ENABLE_PRE_GRASP_REALIGN:
                    log.info("[1-2b] yaw 회전 없이 현재 카메라 자세로 컵 중앙 XY만 재정렬합니다.")
                    aligned = self._align_above_cup(
                        bx, by, cup_theta, grasp_yaw, safe_z, current_ori)
                    if aligned is None:
                        return
                    bx, by, cup_theta, _ = aligned
                    target_ori = current_ori
                else:
                    log.info("[1-2b] XY 재정렬은 비활성화되어 있습니다.")
            else:
                log.warn(
                    "[1-2] 초기 observe에서는 정렬 OK였지만 safe_z에서는 정렬이 충분하지 않습니다. "
                    "기존 XY/yaw 재정렬 루틴으로 전환합니다."
                )
                skip_safe_realign = False

        if not skip_safe_realign and ENABLE_PRE_GRASP_REALIGN:
            ...
            target_ori = get_gripper_pose_by_cup(grasp_yaw, roll_deg=GRASP_ROLL_DEG)
        elif not skip_safe_realign:
            log.info("[Align] 카메라 하향 XY 재정렬은 비활성화되어 있습니다.")

        if skip_safe_realign:
            log.info("[1-3] safe_z 재확인 통과. 추가 yaw 회전 없이 현재 자세로 중앙 파지를 진행합니다.")
        elif ENABLE_MOUTH_UP_ALIGN:
            ...
            if mouth["found"]:
                ...
                if (
                    ...
                    if final_up_error is not None and abs(final_up_error) <= MARKER_EDGE_ALLOW_UP_TOL_RAD:
                        ...
                        return
            elif skip_safe_realign:
                final_up_error = _mouth_up_error(mouth.get("mouth_theta"))
                if final_up_error is None or abs(final_up_error) > INITIAL_OBSERVE_UP_TOL_RAD:
                    log.error(
                        "[1-5] 빨간 점은 없고 노란 벡터도 초기 허용 범위를 벗어났습니다. "
                        "하강하지 않습니다."
                    )
                    return
                log.warn(
                    "[1-5] 빨간 점은 없지만 fast-pick 상태이고 "
                    f"노란 벡터 오차가 {np.degrees(final_up_error):+.1f}도라 하강을 진행합니다."
                )
            else:
        ...
        dx = (CUP_LENGTH_M / 2.0) * np.cos(grasp_yaw)
        dy = (CUP_LENGTH_M / 2.0) * np.sin(grasp_yaw)
        place_x = bx - dx
        place_y = by - dy
