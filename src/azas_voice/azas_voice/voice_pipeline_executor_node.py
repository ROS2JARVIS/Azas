...
class VoicePipelineExecutorNode(Node):
    ...
    def _on_confirmed_decision(self, msg: String) -> None:
        ...
        if not bool(self.get_parameter("enable_hardware_execution").value):
            self._publish_status("dry_run", recipe_colors=recipe_colors, command=command)
            return
        ...
        threading.Thread(
            ...
            args=(proc, recipe_colors),
            daemon=True,
        ).start()

    def _monitor_pipeline(self, proc: subprocess.Popen[str], recipe_colors: str) -> None:
        last_stage = ""
