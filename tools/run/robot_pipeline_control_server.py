...
STEPS = [
    ...
    Step(
        "cup_uprighting_preview",
        "쓰러진 컵 직립화 preview",
        "background",
        "tools/run/run_yolo_cup_uprighting.sh",
        True,
        False,
        "Azas 통합 직립화 flow: preview_only 기본값으로 카메라/YOLO 오버레이만 실행",
    ),
    Step(
        "cup_uprighting_motion",
        "쓰러진 컵 직립화 실제 모션",
        "background",
        "tools/run/run_yolo_cup_uprighting.sh preview_only:=false show_window:=true",
        True,
        True,
        "MoveIt/RG2/HOME 이동 포함. 현장 안전 확인 뒤 카메라 창에서 p 키로 supervised 실행",
    ),
