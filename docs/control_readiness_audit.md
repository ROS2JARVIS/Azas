...
| Pick action no-motion sequence works | `/home/ssu/Azas/tools/smoke_pick_and_align_no_motion.sh` | Fake base-link `PoseStamped` can drive `/azas/pick_and_align` through `WAIT_TUMBLER_POSE`, no-motion pick planning, fake gripper feedback states, `DONE_NO_MOTION`, and `NO_MOTION_PICK_SEQUENCE_OK` without MoveIt, Doosan, or real RG2 commands | Done |
